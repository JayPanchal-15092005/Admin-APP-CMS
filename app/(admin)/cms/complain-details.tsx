...
//   Alert,
//   Image, // 🟢 IMPORT IMAGE
//   Modal, // 🟢 IMPORT MODAL FOR FULL SCREEN
//   ScrollView,
//   StyleSheet,
//   Text,
//   TextInput,
...
//   image_url?: string; // 🟢 ADDED: Image URL to the type
// };
...
//   // 🟢 NEW STATE: Controls whether the full screen image is open
...
//       {/* onRequestClose is what makes the Android physical back button work! */}
//       <Modal
...
//     fontWeight: "600",
//   },
//   // 🟢 NEW STYLES: Thumbnail and Full Screen Image
...
export default function ComplaintDetailsScreen() {
  ...
  const loadDetails = async () => {
    try {
      // 🟢 Verified: This route is correct for your new backend structure
      const res = await fetch(`${API_BASE_URL}/api/admin/complaints/${id}`, {
        headers: getAdminHeaders(adminEmail, adminPassword),
      });
  ...
  const resolveComplaint = async (id: string) => {
    try {
      ...
      // 🟢 FIX: Added /admin/ to the route to match your new adminRoutes.js
      await fetch(`${API_BASE_URL}/api/admin/complaints/${id}/resolve`, {
        method: "POST",
        headers: {
          ...getAdminHeaders(adminEmail, adminPassword),
          "Content-Type": "application/json",
  ...
  // Ensures we only try to render real links, avoiding blank boxes if the DB saved "null" or ""
  const hasValidImage =
      ...
      <View style={styles.wrapper}>
        ...
        >
          <View style={styles.mainCard}>
            ...
            {/* 🟢 FIX: Implemented strict check for valid URLs */}
            {hasValidImage && (
              <View style={styles.section}>
                <View style={styles.sectionHeader}>
                  <Text style={styles.sectionIcon}>📸</Text>

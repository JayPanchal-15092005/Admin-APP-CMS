...
export default function AdminComplaints() {
  ...
  const filteredData = complaints.filter((item) => {
    ...
    const matchesSearch =
      item.department?.toLowerCase().includes(searchLower) ||
      item.complain_detail?.toLowerCase().includes(searchLower) ||
      item.to_whom?.toLowerCase().includes(searchLower) ||
  ...
  return (
    <View style={styles.wrapper}>
      <LinearGradient
        ...
        style={styles.header}
      >
        {/* 🟢 FIXED: Grouped logo/title and grouped action buttons */}
        <View style={styles.topHeaderRow}>
          <View style={styles.titleContainer}>
            <Image
              source={LOGO_IMG}
      ...
      <View style={styles.searchContainer}>
        ...
        <TextInput
          style={styles.searchInput}
          placeholder={`Search ${
            activeFilter !== "All" ? activeFilter.toLowerCase() : "all"
          } complaints...`}
          placeholderTextColor="#9CA3AF"
          value={searchQuery}
          onChangeText={setSearchQuery}
...
const styles = StyleSheet.create({
  ...
  searchInput: { flex: 1, height: 48, fontSize: 15, color: "#1F2937" },
  clearIcon: { padding: 4 },

  // 🟢 NEW: Structure for Top Row
  topHeaderRow: {
    flexDirection: "row",
    justifyContent: "space-between",
    alignItems: "center",
  ...
  // 🟢 NEW: Styling for the group of action buttons (Home + Sign Out)
